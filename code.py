...
for key,value in final_data.iloc[:,[0,2,5]].iterrows():
  if value.frequency > 40:
    ...
    print(value.IP)
    Block_IP.append([value.Clusters, value.IP])
Block_IP[0]
...
for key,value in final_data.loc[:,['IP','frequency','Clusters']].iterrows():
  if value.frequency > 40:
    for i in final_list:
      os.system('sudo iptables -A INPUT -s {} -p tcp --destination-port 80 -j DROP'.format(final_list[i]))
print("IP blocked by system is",final_list)
